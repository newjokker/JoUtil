...
class ArcpyOsgeoUtil(object):
    ...
    @staticmethod
    def add_object_index(shp_path, field_name = 'object_index'):
    ...
    @staticmethod
    def delete_field(shp_path, field_name_list):
        """删除指定的字段"""
        gdal.SetConfigOption("SHAPE_ENCODING", "")
    ...
    @staticmethod
    def polygon_split(shp_path, attribute='fid', save_folder=None, return_mode='SHP'):
        """将 shp 按照属性进行 split"""
        ds = ogr.Open(shp_path)
        lyr = ds.GetLayer(0)
    ...
    @staticmethod
    def set_attribute_filter(in_shp_path, out_shp_path, sql_str, new_lyr_name='new_lyr'):
        """
        进行属性筛选
        :param in_shp_path: 输入 shp
        :param out_shp_path: 输出 shp ，暂时不支持在源文件上修改
        :param sql_str:  用于属性筛选的 SQL 语句
        :param new_lyr_name:  新图层的名字
        :return: 是否成功
        """
        # 找到图层
        ds = ogr.Open(in_shp_path, 0)
        lyr = ds.GetLayer()
        # 使用属性进行筛选
        lyr.SetAttributeFilter(sql_str)
        # 新建图层
        driver = ogr.GetDriverByName("ESRI Shapefile")
        new_ds = driver.CreateDataSource(out_shp_path)
        # 复制图层
        new_ds.CopyLayer(lyr, new_lyr_name)
        new_ds.Destroy()

    @staticmethod
    def set_spatial_filter(in_shp_path, out_shp_path, in_range_shp, new_lyr_name='new_lyr'):
        """
        进行空间筛选
        :param in_shp_path: 输入 shp
        :param out_shp_path: 输出 shp ，暂时不支持在源文件上修改
        ...
        :return: 是否成功
        """
        # fixme 对每一个图层进行筛选，一个图层内所有的 geo 都符合才进行筛选，所以需要将彼此分开的要素放入不同的图层

        #
        ds = ogr.Open(in_shp_path, 0)
        lyr = ds.GetLayer()
        #
        ds_new = ogr.Open(in_range_shp, 0)
        lyr_new = ds_new.GetLayer(0)
        fea = lyr_new.GetFeature(0)
        geo = fea.GetGeometryRef()
        #
