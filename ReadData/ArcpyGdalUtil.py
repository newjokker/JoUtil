...
import os
import shutil
from Jo_util.ReadData.ArcpyOsgeoUtil import ArcpyOsgeoUtil
...
class ArcpyGdalUtil(object):
    ...
    # --------------------------------- 设置更新 -----------------------------
    @staticmethod
    def set_no_data_region_by_mask(tiff_path, mask, assign_band_index= '*',
                                   out_tiff_path = None, assign_no_data_value= None):
        """
        设置无效值掩膜
    ...
    @staticmethod
    def set_array_data(tiff_path, array_data, assign_band_index = '*',
                       out_tiff_path= None):
        """
    ...
    @staticmethod
    def set_array_data_use_assign_algo(tiff_path, func, assign_band_index= '*',
                                       out_tiff_path= None):
        """
        通过传入的方法体对 tiff 进行更新
        :param tiff_path:
    ...
    @staticmethod
    def envelope_intersection(envelopes_list, mode='output_bounds'):
        """
        获取范围的交集
    ...
    # ---------------------------------- 属性获取 ----------------------------
    @staticmethod
    def get_no_data_value(tiff_path):
        """
        获取无效值
        :param tiff_path:
    ...
    @staticmethod
    def get_tiff_infomation(tiff_path, options, return_mode= 'dict'):
        """
    ...
    @staticmethod
    def get_proj(proj_str='WGS84', tiff_path=None):
        """
        返回栅格坐标系
        :param tiff_path: tiff 文件，获取文件的投影
    ...
    @staticmethod
    def get_array_from_tiff_in_assign_size(path, assign_size):
        """从 tiff 中获取采样成指定大小的矩阵"""
        # 参数类型检查
        if isinstance(path, gdal.Dataset):
            dataset = path
        else:
        ...
        im_data = dataset.ReadAsArray(buf_xsize=assign_size[0], buf_ysize=assign_size[1])  # 获取数据
        return im_data
    ...
    def file_operate():
        ...
        driver.CopyFiles()  # 创建数据集副本  属性中有中文会出现问题
